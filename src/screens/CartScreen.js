...
export default function CartScreen() {
  ...
  useFocusEffect(
    useCallback(() => { loadCart().then(setCart); }, [])
  );
  ...
  const clearDone = () => {
    Alert.alert('Effacer', 'Supprimer les articles cochés ?', [
  ...
  const hasDone = cart.some(c => c.done);
    ...
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        <View>
          ...
          {cart.length > 0 && (
            <Text style={s.sub}>{doneCount}/{cart.length} articles cochés</Text>
          )}
        </View>
        {hasDone && (
...
const s = StyleSheet.create({
  ...
  header:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: C.bg2, padding: 18, borderBottomWidth: 1, borderBottomColor: C.border },
